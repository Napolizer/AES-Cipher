...
public class AES {
    ...
    public byte[] encryptBuffer(byte[] buffer, String orgKey) {
        ...
        for (int y = 0; y < 4; ++y) {
            for (int x = 0; x < 4; ++x) {
                buffer[y + x * 4] = stateMatrix.get(x, y);
            }
        }
    ...
    public byte[] decryptBuffer(byte[] buffer, String orgKey) {
        ...
        // Round 0
        keyState = new KeyState(new int[]{keys[0], keys[1], keys[2], keys[3]});
        stateMatrix.xor(keyState);
    ...
    public int rotWord(int word) {
        int first_byte = word & 0xff_00_00_00;
        first_byte >>>= 24;
        int output = word << 8;
        return output | first_byte;
    }

    public int subWord(int word) {
        int first_byte = (0xff_00_00_00 & word) >>> 24;
        int second_byte = (0x00_ff_00_00 & word) >>> 16;
        int third_byte = (0x00_00_ff_00 & word) >>> 8;
        int fourth_byte = 0x00_00_00_ff & word;

        first_byte = applySbox(first_byte) << 24;
        second_byte = applySbox(second_byte) << 16;
        third_byte = applySbox(third_byte) << 8;
        fourth_byte = applySbox(fourth_byte);

        return first_byte | second_byte | third_byte | fourth_byte;
    }

    public int rcon(int word, int round) {
    ...
    // Uses precalculated lookup table
    public int applySbox(int input) {
        int first_half = (0xf0 & input) >> 4;
        int second_half = 0x0f & input;
        return sbox[first_half][second_half];
    }

    public int reverseApplySbox(int input) {
        int first_half = (0xf0 & input) >> 4;
        int second_half = 0x0f & input;
        return inverseSbox[first_half][second_half];
    }
    ...
    public InputStream decipher(InputStream input, String key) {
        //Validate key
        if (key.getBytes().length != 16 &&
